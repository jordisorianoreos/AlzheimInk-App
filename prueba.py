...
def start_session():
    # Si hay un usuario activo, cierra su sesión
    if 'active_user' in session:
        session.pop('active_user', None)
        session.pop('last_activity', None)
    # Inicia una nueva sesión para el nuevo usuario
    ...
    try:
        with open('databases/users.json', 'r', encoding='utf-8') as file:
            ...
            if user and user.get('password') == password:
                session['dni'] = dni  # Guarda el DNI en la sesión
...
def get_table(dni):
    ...
    # Filtrar el DataFrame para incluir solo las filas que corresponden al DNI proporcionado
    df = df[df['DNI'] == dni]
